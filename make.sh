...
releaseDate=$(gh release view v1.3.0 -R freelensapp/freelens --json publishedAt --jq .publishedAt | sed 's/T.*//')
releaseUrl=$(gh release view v1.3.0 -R freelensapp/freelens --json url --jq .url)
installerX64Url=$(gh release view v1.3.0 -R freelensapp/freelens --json assets --jq '.assets[] | select(.name == "Freelens-'"${version}"'-windows-amd64.exe").url')
installerArm64Url=$(gh release view v1.3.0 -R freelensapp/freelens --json assets --jq '.assets[] | select(.name == "Freelens-'"${version}"'-windows-arm64.exe").url')
installerX64Sha256Url=$(gh release view v1.3.0 -R freelensapp/freelens --json assets --jq '.assets[] | select(.name == "Freelens-'"${version}"'-windows-amd64.exe.sha256").url')
installerArm64Sha256Url=$(gh release view v1.3.0 -R freelensapp/freelens --json assets --jq '.assets[] | select(.name == "Freelens-'"${version}"'-windows-arm64.exe.sha256").url')
